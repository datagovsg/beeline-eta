...
from ping_locator import (
    get_kd_tree, list_of_nearest_pings_to_stops, p
)
...
def update_timings_for_trip(date_time, trip_id):
    ...
    predicted_arrival_timings = predict_arrival_timings(trip_id, date_time)

    if not predicted_arrival_timings:
        message = 'No prediction: Insufficient historical data for prediction.'
        update_prediction(trip_id, stop_ids, [message] * len(trip_tripstops))
        return

    # Save and overwrite prediction
    update_prediction(trip_id, stop_ids, predicted_arrival_timings)

    return dict(zip(stop_ids, predicted_arrival_timings))
...
def is_useful_result(ordered_stops, ordered_stops_to_nearest_pings):
    stop_ids, ping_ids_per_stop = transpose(ordered_stops_to_nearest_pings)
    num_pings_per_stop = [len(ping_ids) for ping_ids in ping_ids_per_stop]
    return list(stop_ids) == ordered_stops and all([count > 0 for count in num_pings_per_stop])

def predict_arrival_timings(main_trip_id, date_time):
    ...
    for trip_id in trip_ids:
        ...
        trip_tripstops = get_tripstops(trip_id=trip_id)
        trip_cycle_trip_stops = get_trip_cycle(trip_tripstops.stopId.tolist())
        if trip_cycle_trip_stops != trip_cycle_main_trip_stops:
            continue
        ...
        ordered_stop_to_nearest_pings = list_of_nearest_pings_to_stops(trip_id, date_time=date_time)

        # Check that stops are in order, and there is a nearest ping at every stop
        if not is_useful_result(trip_cycle_trip_stops, ordered_stop_to_nearest_pings):
            continue

        duration_per_stop = []
        for stop_id, ping_ids in ordered_stop_to_nearest_pings:
    ...
    # After getting a few values from prev step, take median.
    ...
    if len(predicted_arrival_timings) > 0:
        first_stop_planned_time = main_trip_tripstops.iloc[0].time.to_pydatetime()
        predicted_arrival_timings[0] = max(predicted_arrival_timings[0], first_stop_planned_time)

    return predicted_arrival_timings
